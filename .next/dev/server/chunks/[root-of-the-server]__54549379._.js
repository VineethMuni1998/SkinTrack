...
Return ONLY valid JSON, no additional text.`;
    try {
        ...
        const resolveProductId = (maybeId, fallbackName)=>{
        ...
        const normalizeIdsArray = (arr)=>Array.isArray(arr) ? arr.map((id)=>typeof id === "string" ? id : "").map((id)=>resolveProductId(id)).filter(Boolean) : [];
        const shareTimeBlock = (ids)=>{
            if (ids.length < 2) return false;
            const times = ids.map((id)=>productById.get(id)?.timeOfDay || "BOTH").map((t)=>t.toUpperCase());
        ...
        const extraRecommendations = crossTimeSynergies.length > 0 ? crossTimeSynergies.map((entry)=>{
...
async function POST(request) {
    try {
        ...
        const products = routine.routineProducts.map((rp)=>({
                id: rp.product.id,
        ...
        // Run AI analysis
        const analysisResult = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$openai$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["analyzeProducts"])(products);
        // Save analysis to database
        const analysis = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].analysis.create({
            data: {
                routineId,
                products: products,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__54549379._.js.map